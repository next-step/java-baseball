...
public class Baseball {
    ...
    public static void runGame() {
        ...
        while (!finished) {
            ...
            finished = gameFinished(source, target);
        }
    }
    ...
    public static boolean gameFinished(String source, String target) {
        int strike = 0;
        int ball = 0;
        for (int i = 0; i < 3; i++) {
    ...
    public static int calculateBall(String source, String target, int targetIndex) {
        if (source.substring(0, targetIndex).contains(target.substring(targetIndex, targetIndex + 1))) {
            return 1;
        }
        if (source.substring(targetIndex + 1).contains(target.substring(targetIndex, targetIndex + 1))) {
            return 1;
        }
        return 0;
    }
