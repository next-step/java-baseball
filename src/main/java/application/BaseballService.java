...
public class BaseballService {
    ...
    public void calculate(BaseballNumber answerNumber, Map<Integer, Integer> answerMap, BaseballNumbers playerBaseballNumbers) {
        if (playerBaseballNumbers.contains(answerNumber) && (playerBaseballNumbers.getBaseballPosition(answerNumber) == answerMap.get(answerNumber.getValue()))) {
            score.addStrike();
        }
        if (playerBaseballNumbers.contains(answerNumber) && (playerBaseballNumbers.getBaseballPosition(answerNumber) != answerMap.get(answerNumber.getValue()))) {
            score.addBall();
