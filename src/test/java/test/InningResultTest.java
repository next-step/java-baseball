...
public class InningResultTest {
    ...
    @Test
    @DisplayName("3스트라이크가 들어왔을 때, 종료가 되는 지 확인하는 테스트")
    void isFinishTrueTest() {
        final LinearStrategy linearStrategy = new LinearStrategy();
        final AnswerNumbers answerNumbers = new AnswerNumbers(linearStrategy);
        final Shots shots = new Shots(answerNumbers);
        ...
        assertThat(inningResult.isFinish()).isEqualTo(true);
    }

    @Test
    @DisplayName("3ball이 들어왔을 때, 종료가 되지 않는 지 확인하는 테스트")
    void isFinishFalseTest() {
        ...
        assertThat(inningResult.isFinish()).isEqualTo(false);
    }
