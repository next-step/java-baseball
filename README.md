...
- [ ] 가지고 있는 _한 자리 게임 숫자(GameDigit)_ 를 인덱스 별로 제공한다.
- [ ] 3개의 중복 되지 않는 _한 자리 게임 숫자(GameDigit)_ 를 제공한다.
...
- ([GameDigit] digits)
- GameDigit 일급 컬렉션
- 항상 3개의 서로 다른 _한 자리 게임 숫자(GameDigit)_ 를 가지고 있다.
- [ ] 생성 시 3개의 _한 자리 게임 숫자(GameDigit)_ 를 가지지 않으면 예외를 발생시킨다.
- [ ] 생성 시 중복된 _한 자리 게임 숫자(GameDigit)_ 가 있으면 예외를 발생시킨다.
- [ ] _한 자리 게임 숫자(GameDigit)_ 를 받아서 자신이 가진 게임 숫자에 대해 _매치 타입(MatchType)_ 을 반환한다.

### BaseballGame (숫자 야구 게임)

- 한 게임당 랜덤으로 새롭게 생성된 `정답 게임 숫자` 하나를 가진다.
- [ ] 새로운 게임을 시작하면 새로운 랜덤한 _게임 숫자(GameNumber)_ 
- [ ] 유저가 입력한 _게임 숫자(GameNumber)_ 를 받아 올바른 _정답 추측 결과(GuessResult)_ 를 제공한다.
